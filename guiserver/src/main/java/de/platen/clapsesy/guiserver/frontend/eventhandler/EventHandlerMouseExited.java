...
public class EventHandlerMouseExited extends EventHandlerMouse {
    ...
    public EventHandlerMouseExited(final GuiElement guiElement, final EventSenderMouseExited eventSenderMouseExited) {
        super(guiElement);
        eventHandler = event -> {
            try {
                ...
                if (!guiElement.isInactive()) {
                    ...
                    if (hasToSend) {
                        eventSenderMouseExited.sendEvent(guiElement.getElementId().get(), xRelative, yRelative, null,
